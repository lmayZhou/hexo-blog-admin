...
var config = {
    ...
    right: -232,
    api: {
        friend: 'plugins/layer/layim/data/friend.json', //好友列表接口
        group: 'plugins/layer/layim/data/group.json', //群组列表接口
        chatlog: 'plugins/layer/layim/data/chatlog.json', //聊天记录接口
        groups: 'plugins/layer/layim/data/groups.json', //群组成员接口
        sendurl: '' //发送消息接口
    },
    user: { //当前用户信息
        name: '游客',
...
xxim.popchat = function(param){
    ...
    log.html = '<div class="layim_chatbox" id="layim_chatbox">'
            ...
            +'    <a href="'+ param.url +'" class="layim_face" target="_blank"><images src="'+ param.face +'" ></a>'
...
xxim.tabchat = function(param){
    ...
    xxim.chatbox.find('.layim_face>images').attr('src', param.face);
...
xxim.getGroups = function(param){
    ...
    config.json(config.api.groups, {}, function(datas){
        if(datas.status === 1){
            ...
            if(lens > 0){
                for(; ii < lens; ii++){
                    str += '<li data-id="'+ datas.data[ii].id +'" type="one"><images src="'+ datas.data[ii].face +'" class="xxim_oneface"><span class="xxim_onename">'+ datas.data[ii].name +'</span></li>';
                }
            } else {
                        ...
                        + function(){
                            if(type === 'me'){
                                return '<span class="layim_chattime">'+ param.time +'</span>'
                                       ...
                                       +'<images src="'+ param.face +'" >';
                            } else {
                                return '<images src="'+ param.face +'" >'
                                       +'<span class="layim_chatname">'+ param.name +'</span>'
                ...
                if(index !== 2){
                    for(; i < myflen; i++){
                        ...
                        for(var j = 0; j < item.length; j++){
                            str += '<li data-id="'+ item[j].id +'" class="xxim_childnode" type="'+ (index === 0 ? 'one' : 'group') +'"><images src="'+ item[j].face +'" class="xxim_oneface"><span class="xxim_onename">'+ item[j].name +'</span></li>';
                        }
                        str += '</ul></li>';
                    }
                } else {
                    str += '<li class="xxim_liston">'
                        +'<ul class="xxim_chatlist">';
                    for(; i < myflen; i++){
                        str += '<li data-id="'+ datas.data[i].id +'" class="xxim_childnode" type="one"><images src="'+ datas.data[i].face +'"  class="xxim_oneface"><span  class="xxim_onename">'+ datas.data[i].name +'</span><em class="xxim_time">'+ datas.data[i].time +'</em></li>';
                    }
                    str += '</ul></li>';
                }
                myf.html(str);
